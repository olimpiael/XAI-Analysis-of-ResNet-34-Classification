...
from pathlib import Path          # Path() w zapisie plików
import random                     # random.sample() do baseline’ów
...
if LRP_:
    ...
    classes_per_row = 3                 # <- 3 klasy na wiersz
    rows = math.ceil(n_classes / classes_per_row)
    cols = 2 * classes_per_row          # <- 6 kolumn: heat/overlay x3

    if typ:
        assert len(selected_classes) == 14, "Ten layout zakłada 14 klas."
    else:
        assert len(selected_classes) == 7, "Ten layout zakłada 7 klas."

    fig, axes = plt.subplots(rows, cols, figsize=(cols * 3.5, rows * 2.6))

    # Ujednolicenie wymiarów axes dla przypadku rows==1
    if rows == 1:
    ...
    for idx, cls in enumerate(selected_classes):
        ...
        axes[row, col_heat].set_title(f"{cls}")
        axes[row, col_heat].axis('off')

        # Colorbar przy HEATMAPIE
        divider = make_axes_locatable(axes[row, col_heat])
    ...
    # Wyłącz niewykorzystane osie (np. dla 7 klas przy 3/wiersz zostają puste 4 osie)
    for j in range(n_classes, rows * classes_per_row):
        r = j // classes_per_row
        cblock = j % classes_per_row
        axes[r, cblock * 2].axis('off')
        axes[r, cblock * 2 + 1].axis('off')

    plt.tight_layout(rect=[0, 0, 1, 0.96])
    fname = f"lrp_{layer_name}_14klas.png"
    plt.savefig(os.path.join(OUT_DIR, fname))
    plt.show()

# ======= GradCAM - różne rodzaje =======
# class GradCAM:
...
#         weighted_activations = self.activations * pooled_grad[None, :, None, None]
#         heatmap = weighted_activations.sum(dim=1).squeeze()
#         heatmap = torch.relu(heatmap)
#         heatmap /= (heatmap.max() + 1e-8)
#         return heatmap.cpu().numpy()
if GradCAM_:
    print("Generowanie map Grad-CAM...")
    class GradCAM:
        def __init__(self, model, target_layer):
            self.model = model
            self.target_layer = target_layer
    ...
    print("Porównanie: Grad-CAM vs Layer-CAM vs Score-CAM")

    methods = {
        "Grad-CAM": GradCAM(model, model.layer1[-1].relu),
    #"Grad-CAM++": GradCAMpp(model, model.layer1[-1].relu),
        "Score-CAM": ScoreCAM(model, model.layer1[-1].relu),
        #"Eigen-CAM": EigenCAM(model, model.layer1[-1].relu),
        #"XGrad-CAM": XGradCAM(model, model.layer1[-1].relu),
        #"FullGrad": FullGradCAM(model, model.layer1[-1].relu),
...
if DeepSHAP_:
    ...
    # all_classes = list(class_to_idx.keys())[:n_cols]  

    # # -------- STRONA 1 (wiersze 0–6) --------
    # start_row = 0
    # sampled = []
    # for kl in all_classes[start_row : start_row + n_rows]:
    ...
    #         ax = axes[row, col]
    #         im = ax.imshow(attr, cmap='bwr', alpha=1, vmin=-vmax, vmax=+vmax)
    #         ax.set_xticks([]); ax.set_yticks([])
    #         if row == 0:
    #             ax.set_title(klasa_kolumny, fontsize=8, rotation=60, ha="left")
    #         if col == 0:
    ...
    # plt.show()
    # fig.savefig(OUT_DIR / "deepshap_page1.png", dpi=300, bbox_inches="tight")
    # # -------- STRONA 2 (wiersze 7–13) --------
    # start_row = 7
    # sampled = []
    # for kl in all_classes[start_row : start_row + n_rows]:
    #     if kl in correct_samples:
    ...
    # fig.colorbar(im, cax=cbar_ax, label="SHAP value")
    # plt.show()
    # fig.savefig(OUT_DIR/ "deepshap_page2.png", dpi=300, bbox_inches="tight")
    n = 14  
